...
// 複数店舗のターゲット設定（ここに店舗をどんどん追加できます）
const targetStores = [
  {
    store_name: 'パラッツォ船橋店',
    url: 'https://min-repo.com/tag/パラッツォ船橋店/'
  },
  {
    store_name: 'マルハン千葉みなと店',
    url: 'https://min-repo.com/tag/マルハン千葉みなと店/'
  }
];

async function scrapeMinRepoData() {
  console.log('【みんレポ】ステルスモードでブラウザを起動中...');
  ...
  try {
    for (const target of targetStores) {
      ...
      try {
        ...
        const parsedData = await page.evaluate((storeName) => {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (/^\d{4}[\/\-]\d{2}[\/\-]\d{2}/.test(line)) {
              const dateStr = line.substring(0, 10).replace(/-/g, '/');
        ...
        if (parsedData.length > 0) {
          console.log(`${target.store_name} のデータをSupabaseへ保存・更新中...`);
    ...
    await browser.close();
    console.log('\nすべての処理が終了しました。ブラウザを閉じました。');
  }
}

scrapeMinRepoData();

export { scrapeMinRepoData };